...
with open(filename, 'w') as f:
    print(f"{ruleset1['name']} Unique Rules", file=f)
    print(f"{ruleset1['name']} Unique Rules")
    for rule in rules1:
        if not rule["dupe"]:
            print(f"\t{rule['ruleId']}\t{rule['name']}", file=f)
            print(f"\t{rule['ruleId']}\t{rule['name']}")
    print(f"{ruleset2['name']} Unique Rules", file=f)
    print(f"{ruleset2['name']} Unique Rules")
    for rule in rules2:
        if not rule["dupe"]:
            print(f"\t{rule['ruleId']}\t{rule['name']}", file=f)
            print(f"\t{rule['ruleId']}\t{rule['name']}")
    print(f"Rules Common to both {ruleset1['name']} and {ruleset2['name']}", file=f)
    print(f"Rules Common to both {ruleset1['name']} and {ruleset2['name']}")
    for rule in rd:
        if rule['ruleset2Id'] != 0:
            print(f"\t{rule['ruleId']}:{rule['ruleset2Id']}\t{rule['name']}", file=f)
